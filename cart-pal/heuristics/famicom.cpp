...
FamicomCartridge::FamicomCartridge(const uint8_t* _data, uint size) {
  const Natural<8>* data = (Natural<8>*)_data;
  if(size < 16) return;
  if(data[0] != 'N') return;
  if(data[1] != 'E') return;
  if(data[2] != 'S') return;
  ...
  markup.append("board region=", region == (uint)Region::PAL ? "pal" : "ntsc-j", " ");
